...
from BFSSolver import bidirectional_breadth_first_search, State
...
class TestSolver(unittest.TestCase):
    def test_start_state(self):
        self.assertEqual(bidirectional_breadth_first_search(State()), [])
    ...
    def test_one_turn(self):
        ...
        self.assertEqual(bidirectional_breadth_first_search(starting_state), ["R'"])
    ...
    def test_two_turns(self):
        ...
        starting_state = State(starting_permutation)
        self.assertEqual(bidirectional_breadth_first_search(starting_state), ["D'", "R'"])
    ...
    def test_maximum_scramble(self):
        ...
        for move in moves:
            starting_permutation = starting_permutation.permutation_after_move(move)
        starting_state = State(starting_permutation)
        self.assertEqual(len(bidirectional_breadth_first_search(starting_state)), 14)

    def test_scramble_error(self):
        invalid_permutation = Permutation([i for i in range(24)])
        invalid_state = State(invalid_permutation)
        with self.assertRaises(ValueError):
            bidirectional_breadth_first_search(invalid_state)
